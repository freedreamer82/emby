...
Timer::Timer(TimerSize ms, bool periodic,Type timeType)
{
    auto ticks = millisToTicks(ms);
    if (ticks == 0)
        ticks = portMAX_DELAY;
                      ...
                      TimerSize CmdTimeout)
{
    ...
    if (EmbySystem::isInInterrupt())
    {
        auto ret = (xTimerChangePeriodFromISR(m_impl.m_handle, NewPeriod, &xHigherPriorityTaskWoken) == pdFALSE) ? false
                                                                                                                 : true;
        portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
        return ret;
    }
